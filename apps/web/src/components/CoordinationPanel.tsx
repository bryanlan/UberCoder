...
export function CoordinationPanel({ checkout }: { checkout: string }) {
  const query = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/coordination?${new URLSearchParams({ checkout })}`, { credentials: 'include' });
      if (!response.ok) throw new Error('Coordination is unavailable. Editing claims and message delivery cannot be verified.');
  ...
  const activeAssignments = data.assignments.filter((assignment) => assignment.status === 'active' || assignment.status === 'waiting' || data.claims.some((claim) => claim.assignmentId === assignment.id));
  const pendingMessages = data.messages.filter((message) => !message.acknowledgedAt && activeAssignments.some((assignment) => assignment.id === message.recipientId));
  ...
  return (
    <details className="mx-4 my-2 rounded-xl border border-slate-700 bg-slate-900 text-sm text-slate-200 lg:mx-6">
      <summary className="cursor-pointer px-3 py-2">Work in this repository · {activeAssignments.length} assignments · {pendingMessages.length} messages awaiting acknowledgement</summary>
        ...
        <ul className="space-y-2" aria-label="Active assignments">
          {activeAssignments.map((assignment) => (
            <li key={assignment.id}>
              ...
              {data.claims.filter((claim) => claim.assignmentId === assignment.id).map((claim) => <div key={`${claim.checkout}:${claim.path}`} className="break-all text-xs text-amber-200">Editing: {claim.path}{assignment.status === 'disconnected' || assignment.status === 'finished' ? ' · unfinished changes need reconciliation' : ''}</div>)}
            </li>
          ))}
        </ul>
