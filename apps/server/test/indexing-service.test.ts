...
describe('IndexingService', () => {
  ...
  it('adopts pending conversations into real history nodes and hides the pending alias from the tree', async () => {
    ...
    db.putPendingConversation({
      ...
      createdAt: '2026-03-06T23:00:00.000Z',
      updatedAt: '2026-03-07T00:00:30.000Z',
  ...
  it('uses the newest meaningful session timestamp as synthetic conversation recency', async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'agent-console-indexing-'));
    const db = new AppDatabase(path.join(tempDir, 'agent-console.sqlite'));
    db.upsertBoundSession({
      id: 'session-restored-live-activity',
      provider: 'claude',
      projectSlug: 'demo',
      conversationRef: 'restored-live-activity',
      tmuxSessionName: 'ac-claude-demo-restored-live-activity',
      status: 'bound',
      title: 'Restored live activity',
      startedAt: '2026-03-07T00:00:00.000Z',
      updatedAt: '2026-06-17T18:31:00.000Z',
      lastCompletedAt: '2026-03-07T00:10:00.000Z',
      lastActivityAt: '2026-06-17T18:20:00.000Z',
      lastOutputAt: '2026-06-17T18:21:00.000Z',
      eventLogPath: path.join(tempDir, 'events.jsonl'),
    });
    ...
    const conversation = indexing.getTree().projects[0]?.providers.claude.conversations[0];
    expect(conversation).toMatchObject({
      ref: 'restored-live-activity',
      updatedAt: '2026-06-17T18:21:00.000Z',
    });
    db.close();
  });
