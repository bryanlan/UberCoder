...
function needsAnchoredLastHourSummary(summary) {
  return summary?.status !== 'ready'
    || !summary.recentChangesSummary
    || !summary.windowEndAt
    || !summary.lastInteractionAt
    || !timestampsMatch(summary.windowEndAt, summary.lastInteractionAt);
}
...
async function main() {
  ...
  try {
    ...
    const needsBackfill = sessions.filter((session) => {
      const summary = existing.get(session.id);
      return needsAnchoredLastHourSummary(summary);
    });

    console.log(`Visible Work mode sessions: ${sessions.length}`);
    console.log(`Missing or stale anchored last-hour summaries before run: ${needsBackfill.length}`);
    if (dryRun) {
      console.log('Dry run only; no summaries generated.');
      return;
    }
    if (needsBackfill.length === 0) {
      console.log('No missing or stale summaries; nothing to backfill.');
      return;
    }
    ...
    await service.runOnce({
      bootstrap: true,
      force: true,
      sessionIds: needsBackfill.map((session) => session.id),
    ...
    console.log(`Missing or stale anchored last-hour summaries after run: ${remaining.length}`);
  } finally {
    db.close();
  }
}
